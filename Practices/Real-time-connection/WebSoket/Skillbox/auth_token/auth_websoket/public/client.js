...
loginContainer.addEventListener("submit", (event) => {
    ...
    fetch("/login", {
        ...
        .then(({ token }) => {
            //? у відповідь на успішну авторизацію отримуємо токен
            const wsProto = location.protocol === "https:" ? "wss:" : "ws:"
            client = new WebSocket(
                `${wsProto}//${location.host}?token=${token}` //? відкриваємо з'єднання сокета із строкою токена, по якому ми визначаємо користувача
            )
